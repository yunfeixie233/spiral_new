...
class CoupEnv(ta.Env):
    ...
    def reset(self, num_players: int, seed: Optional[int] = None) -> None:
        ...
        assert 2 <= num_players <= 6, f"Coup requires 2-6 players, got {num_players}"
        
        self.state = ta.State(num_players=num_players, seed=seed)
        self.state.error_allowance = 3
        self.state.prevent_player_change = False
        self.state.made_invalid_move = False
        self.state.error_count = 0
        
        # Add missing methods to State
        def set_winners(player_ids: List[int], reason: str):
            self.state.rewards = {pid: (1 if pid in player_ids else -1) for pid in range(num_players)}
            for pid in range(num_players):
                self.state.game_info[pid]["reason"] = reason
            self.state.done = True
        
        def set_invalid_move(player_id: int, reason: str):
            ...
            self.state.add_observation(message=f"Invalid move by Player {player_id}: {reason}", observation_type=ta.ObservationType.GAME_ADMIN, to_id=player_id)
        
        self.state.set_winners = set_winners
        self.state.set_invalid_move = set_invalid_move
        ...
        # Initialize textarena state using standard_resets
        self.state.standard_resets(game_state=game_state, player_prompt_function=self._gen_initial_prompt)
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        self.state.add_observation(message=action, observation_type=ta.ObservationType.PLAYER_ACTION, from_id=self.state.current_player_id, to_id=-1)
    ...
    def _update_action_metadata_for_query_for_block_or_challenge_phase(self, action: CoupActionType):
        ...
        if action is CoupActionType.PASS:
            ...
            raise ValueError(f"Invalid action: {action}, Player #{action_metadata.source_player_id} is attempting to {action_metadata.action_type}, you must either {block_options_str}{bullshit_str}or [[PASS]]")
    ...
    def _execute_current_action(self):
        ...
        if source_player_observation:
            self.state.add_observation(message=source_player_observation, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=curr_action.source_player_id)
        if target_player_observation is not None:
            self.state.add_observation(message=target_player_observation, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=curr_action.target_player_id)
    ...
    def _execute_showdown_on_bullshit(self):
        ...
        if is_honest:
            ...
            is_assassination = self.state.game_state["action_metadata"].action_type is CoupActionType.Assassinate
            if is_assassination:
                self.state.game_state["coins"][challenged_player_id] += 3
                self.state.game_state["treasury_coins"] -= 3
            
            challenged_remaining_cards = self.state.game_state["hidden_hand"][challenged_player_id]
            ...
            challenged_message = f"You were challenged on your {challenged_card} claim by Player #{challenger_player_id}, Since you did not have a {challenged_card} card, you lost your {card_lost_by_challenged} card." + \
                (f" Your 3 coins for the assassination attempt have been refunded." if is_assassination else "") + \
            ...
            challenger_message = f"You just successfully challenged Player #{challenged_player_id} on their {challenged_card} claim! " + \
                ...
                (f" They were refunded their 3 coins." if is_assassination else "")
        ...
        self.state.add_observation(message=challenged_message, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=challenged_player_id)
        self.state.add_observation(message=challenger_message, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=challenger_player_id)
    ...
    def _execute_showdown_on_blocker_bullshit(self):
        ...
        self.state.add_observation(message=blocker_msg, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=blocker_player_id)
        self.state.add_observation(message=challenger_msg, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=challenger_player_id)
    ...
    def _execute_exchange_action(self):
        ...
        self.state.add_observation(message=source_observation, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=player_id)
    ...
    def _broadcast_observations(self, other_player_observations: str, exclude_player_ids: Optional[List[int]] = None):
        ...
        for pid in range(self.state.num_players):
            if pid in exclude_player_ids:
                continue
            self.state.add_observation(message=other_player_observations, observation_type=ta.ObservationType.GAME_MESSAGE, from_id=ta.GAME_ID, to_id=pid)
    ...
    def _send_call_to_action_prompt(self) -> None:
        ...
        self.state.add_observation(message=msg, observation_type=ta.ObservationType.GAME_BOARD, from_id=ta.GAME_ID, to_id=self.state.current_player_id)
    ...
    def _advance_turn(self):
        ...
        self.state.current_player_id = next_pid
